...
Rooted at `apps/terminal-turmoil/`; `scripts/` (play-testing harness + `generate_data/`) is a sibling of `src/`.

> **Path convention.** Skill docs may still cite bare `src/...` paths. Resolve them as `apps/terminal-turmoil/src/...` for app/story code (everything in the tree below), **except** the generic engine modules that moved to `@tt/core` — `commands` (parser/registry/generic builtins), `filesystem`, `git`, `dbt`, `snowflake`, `session`, `suggestions`, `terminal` (paneTypes/copyMode/tmuxConfig/ansiPalette/zshHistory), and `PaneDividers` — which now live at `packages/core/src/...`. Story-coupled bits (`engine/mail`, `engine/piper`, `engine/prompt`, `engine/narrative`, `engine/commands/builtins/*` story commands) stayed in the app.

```
apps/terminal-turmoil/src/
...
├── state/                  # Zustand store (gameStore.ts), save system (saveManager, saveTypes) — tmux window/pane tree model moved to @tt/core/terminal/paneTypes
...
- **Multi-terminal windows + panes**: The terminal multiplexer is a true tmux model — **windows** (the tabs in the status line, up to `MAX_WINDOWS=5`) each own a **binary tree of panes**, and each pane (a `PaneLeaf`) is a full shell with its own xterm instance, cwd, computerId, and session. The tree model + pure helpers live in `@tt/core/terminal/paneTypes` (`PaneNode = PaneLeaf | PaneSplit`, `WindowState { id, root, activePaneId }`, plus `splitNode`/`collapsePane`/`prunePanesByComputer`/`paneRects`/`nodeBox`/`focusDirectionTarget`/`nearestResizableSplit`/`setSplitRatio`/`nudgeSplitRatio`/`serializeWindow`/`rebuildWindow`). Store state is `windows[]` + `activeWindowId` (the focused pane is each window's `activePaneId`); derive the focused leaf with `getActiveLeaf(state)`/`getActivePaneId(state)` exported from `gameStore.ts`. Persisted as `SaveData.windows` (`SavedWindowState` = serialized tree + `activePaneIndex` DFS index) + `activeWindowIndex`. `tabs_unlocked` is set `true` in `createInitialState()`, so windows/panes/copy-mode are available from game start. **Split bindings are hardcoded** (`TabManager.tsx` `handleCtrlBAction` + the prefix branch of each pane's `onData`): `<prefix> |` = `splitPane(activePaneId, "h")` (side-by-side, vertical divider), `<prefix> -` = `"v"` (stacked); the new pane inherits the focused pane's cwd+computer (tmux `-c "#{pane_current_path}"`). `<prefix> + arrows` = `focusDirection` (geometric select-pane), `<prefix> o` = `cyclePane`. Tmux defaults `%`/`"` are intentionally **not** bound. **Vim-style pane binds are file-driven**, parsed from `~/.tmux.conf` `bind`/`bind-key` lines by `parseTmuxBindings()` (→ `bindingsRef`, looked up case-sensitively in the `onData` prefix branch *before* the hardcoded arrow/`handleCtrlBAction` paths): `bind h/j/k/l select-pane -L/D/U/R` → `focusDirection` (alongside the still-hardcoded arrows), `bind -r H/J/K/L resize-pane -L/D/U/R N` → `applyResize`. Resize is **cell-based** (tmux moves borders in grid cells): `applyResize` finds the nearest ancestor split of the right orientation (`nearestResizableSplit`, L/R→`h`, U/D→`v`), converts N cells → a ratio delta via the pane's live cell size (`containerEl.clientWidth/term.cols`) and the split's pixel box (`nodeBox` × wrapper px), and calls `nudgeSplitRatio` (moves the divider toward the arrow; no-op if no such split). The `-r` flag enables a 500ms **repeat window** (`repeatModeRef`/`repeatTimerRef`): after a repeatable resize bind you can re-tap H/J/K/L without re-pressing the prefix; any other key (or timeout) ends it. Window-level chords stay `<prefix> C` (new window) `/N/P` (next/prev, with `.`/`,` as aliases) `/1-5` (jump, **window**-indexed) `/R` (rename the active window via an inline `(rename-window) <text>` status-bar prompt → `renameWindow`; Enter applies, Esc cancels, empty reverts to the derived label — the custom name is stored as `WindowState.name` and persisted through `serializeWindow`/`rebuildWindow`). `<prefix> X` kills the **focused pane** via a tmux `confirm-before` prompt rendered in the status bar (allowed unless it's the only pane of the only window; killing a window's last pane drops the window). `MAX_PANES_PER_WINDOW=6`. **Rendering** is hybrid: xterm pane containers are imperative, long-lived, keyed by pane id, and positioned **absolutely** from `paneRects(activeWindow.root)` (avoids the re-parenting that a nested-flex tree would inflict on xterm's canvas); only the active window's panes are visible (others `display:none`). A single wrapper `ResizeObserver` + a layout effect fit every visible pane and call `resizePaneSession(paneId)` on size change; `PaneDividers.tsx` overlays draggable seams that call `resizePane(splitId, ratio)`. The active pane shows a 1px accent outline; clicking a pane fires `setActivePane` + `term.focus()` (the single-focused-xterm invariant that keeps `handleInput` routing to the right session — `sessionMapRef` and the global cwd/computer refs are keyed on `activePaneId`). **Copy mode** (`<prefix> [`) is per-pane (each pane owns its own `CopyModeController` in `src/engine/terminal/copyMode.ts`) — `[` enters it on the focused pane and the single overlay indicator reflects the active pane; clipboard via `src/lib/clipboard.ts`, key-hint overlay toggled with `?` and persisted as `copyModeHelpHidden`. It works over any session: inline sessions navigate the real scrollback, alt-screen sessions (`nano`/`less`/`piper`, per `sessionUsesAltScreen()` in `src/engine/session/types.ts`) are confined to the visible screen and get a `resize()` redraw on exit. The tab bar (`TabBar.tsx`) is styled as a tmux status line (`[session]` block, `index:host:dir` windows labeled by their focused pane with a `(n)` pane count, 1-based, `*` on current); its "+" dropdown offers home plus only machines with at least one open pane, so preserved-but-disconnected state (mid-shift soft disconnect) is reachable only via `ssh`/`coder`, never one click from "+". The prefix key and status-bar colors are user-configurable via `~/.tmux.conf` on the home PC, parsed live by `parseTmuxPrefix()`/`parseTmuxTheme()`/`parseTmuxBindings()` in `src/engine/terminal/tmuxConfig.ts` (default Ctrl+Space; named colors resolve against `src/engine/terminal/ansiPalette.ts`; `DEFAULT_TAB_BAR_THEME` is the fallback; the seeded `.tmux.conf` in `story/filesystem/home/dotfiles.ts` ships the vim nav/resize binds). The prefix label reaches `help` via `CommandContext.tabPrefixLabel`. Per-computer command queue serializes FS mutations to prevent TOCTOU races (a window can hold panes on different computers; only the submitting pane is input-gated via `busyPaneIdRef`).
