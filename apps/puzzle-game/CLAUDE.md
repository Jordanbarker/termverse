...
- **`challenges/`** — declarative challenge framework. `types.ts` (`Challenge`, `Step`, `PuzzleSnapshot`), `registry.ts` (ordered `CHALLENGES[]`), and one file per challenge (`panes-split.ts`, `windows-create.ts`, `git-first-commit.ts`). See the `apps/puzzle-game:challenges` skill.
