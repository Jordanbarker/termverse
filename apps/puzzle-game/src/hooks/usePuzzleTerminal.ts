...
/** Coloured prompt for a given pane, e.g. `player@puzzle:~/project$ `. */
export function getPrompt(paneId: string): string {
  const { windows } = usePuzzleStore.getState();
  const win = windows.find((w) => findLeaf(w.root, paneId));
  const cwd = win ? findLeaf(win.root, paneId)?.cwd ?? HOME_DIR : HOME_DIR;
  return `\x1b[32m${USERNAME}@puzzle\x1b[0m:\x1b[34m${shortenCwd(cwd)}\x1b[0m$ `;
}
 ...
 * multi-machine routing (none are needed by the puzzle challenges).
 */
...
export function usePuzzleTerminal() {
