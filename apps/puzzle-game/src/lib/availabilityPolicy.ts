...
/**
 * Per-challenge command allowlist (puzzle-game). Each challenge may declare a
...
function isAvailable(commandName: string): boolean {
  const challenge = CHALLENGES[usePuzzleStore.getState().challengeIndex];
...
export const PUZZLE_AVAILABILITY_POLICY: AvailabilityPolicy = { isAvailable, unavailableMessage };

setAvailabilityPolicy(PUZZLE_AVAILABILITY_POLICY);
