...
- **Security tripwire / forced termination**: `terminated_for_misconduct`, `termination_reason` — set by `runTerminationTransition` when the player trips a file-integrity / DLP rule on nexacorp. Locks `ssh nexacorp` (`Permission denied (publickey,password).`) and drives the three home termination emails. See "Security tripwires + forced termination" below for the full table.
...
|---|---|---|---|
| home | nexacorp | set | `runTerminationTransition(reason)` (security tripwire — checked first) |
...
On nexacorp, certain destructive operations attach a `securityViolation` field to `CommandResult` and cause `applyResult.computeEffects()` to set `effects.transitionTo = "home"` plus `effects.terminationReason = <kind>`. The dispatcher's first branch routes that to `runTerminationTransition`, which prints `Connection to nexacorp closed by remote host.`, sets `storyFlags.terminated_for_misconduct = true` and `storyFlags.termination_reason = <kind>`, rebuilds home, and delivers the matching termination email via a synthesized `{ type: "terminated", detail: <kind> }` event (see `email` skill on `after_event_detail`). After termination, `ssh nexacorp` refuses with `Permission denied (publickey,password).` — soft bad ending; the main mystery thread is dead but the player can keep using home.

Tripwire patterns (all scoped to `activeComputer === "nexacorp"` in `src/story/security.ts`):

| Pattern | Kind | Trips on |
|---|---|---|
| `/var/log/*.log` and `*.log.bak` | `log_tampering` | `rm`, restrictive `chmod`, `> redirect` |
| Under `/srv/leadership/` | `leadership_destruction` | `rm`, restrictive `chmod` (when dest is NOT also under leadership/home for `mv`) |
| `/srv/leadership/**` → `/home/{username}/**` | `exfiltration` | `cp`, `mv` |
