...
**Don't maintain a hand-curated flag table here — it rots.** The authoritative list is `STORY_FLAG_NAMES` in `src/story/storyFlags.ts` (100+ entries, growing). Each flag is defined exactly once in that array, and the integrity test at `src/story/__tests__/storyIntegrity.test.ts` will fail any reference to an undefined flag. Flags set programmatically (via `setStoryFlag` in hooks/transitions) must be registered there too, even though the type system won't force it (`setStoryFlag` takes a plain `string` key).
...
### Finding flags by purpose

`STORY_FLAG_NAMES` is organized under comment headers by arc (home core flow, command unlocks, Olive's quests, NexaCorp onboarding & gating, investigation breadcrumbs, Day 1/Day 2 quests, security tripwire, Chapter 3 endgame). Don't mirror that list here; when adding a flag, append it under the matching comment group in `src/story/storyFlags.ts`. Two groups with non-obvious semantics:

- **Termination detail flags** (`termination_path`, `termination_command`, `termination_descendant_count`, `termination_dest_path` — the last set on exfiltration only): set by `runTerminationTransition`, read by the termination email bodies so HR can name the actual command, path, and file count. See "Security tripwires + forced termination" below.
- **Accusation carrier flags** (`accused_edward/sarah/erik/nobody`, `accusation_made`): persist past Chapter 3 so a future Chapter 4 board-meeting scene can branch on the player's pick (Chapter 4 is planned, not implemented).
...
`src/story/chapters.ts` is the source of truth — three chapters today: **chapter-1** "The Offer" (home-PC onboarding before accepting the offer), **chapter-2** "Onboarding" (NexaCorp Day 1), **chapter-3** "In Production" (Day 2). Don't mirror the objective tree here: read `chapters.ts` for the current objectives/groups, and `docs/storyboard/chapter-{1,2,3}.md` for the narrative beats behind them.

Behaviors not obvious from the definitions alone:

- **`anon_tip_quest`** (chapter-3) is visible after `anon_tip_quest_started` (set by `command_executed:shutdown` at end of Day 1), but its scaffold children only appear if the player accepts the USB (`accepted_usb_drive`).
- **`loose_thread_quest`** (chapter-3) opens only when **both** `read_usb_note` AND `chipinfra_visited` are true (see "Cross-arc cascades" above). Its final cover-tracks step is optional and only ticked by the `rm` scrub; other scrubs (nano, `>` redirect) still suppress the HR email without ticking the HUD objective.
- **`marcus_endgame_quest`** (chapter-3, the accusation → debrief → credits flow) spans `src/story/piper/messages/marcus.ts`, `src/story/marcusDebrief.ts`, `src/story/emails/home.ts`, `exit.ts`/`shutdown.ts` builtins, `useComputerTransitions.ts`, and `src/lib/ascii.ts`. Key beats: Marcus's 4-way accusation DM sets an `accused_*` carrier flag plus `accusation_made`; `exit` from NexaCorp (gated on `accusation_made`) emits `command_executed:exit_day2_logoff` → `returned_home_day2` → delivery of the `marcus_board_debrief` email (body branched 4 ways by `getMarcusDebrief(storyFlags)`); reading it via `mail` fires `read_board_debrief_day2`, which completes the quest and routes home `shutdown` to the endgame branch of `runShutdownTransition` (prints `getEndgameCreditsBlock()`, sets `game_ended`, leaves `gamePhase` at `"transitioning"` so input stays suppressed). Before the debrief is read, a mid-Day-2 home `shutdown` is just a cosmetic reboot.
...
| `grep`, `find`, `wc`, `sort`, `uniq`, `head`, `tail`, `diff` | `returned_home_day1` | end of Day 1 (these were learned at NexaCorp; only available at home after the player has been there). `exit` from NexaCorp is **not** hard-gated: it always returns the player to the home shell (true-to-life `exit`; they can `ssh` back to finish). A mid-shift logoff is a **soft disconnect** (`runExitToHome` delegates to `runExitToParent(term, "home")` when `isEndOfDayExit()` is false): tabs and work-machine `computerState` survive, no flags are set, and `ssh` back reattaches without the boot animation — so a mid-shift logoff cannot skip Day 1. Only a genuine end-of-day exit (`isEndOfDayExit()`: Day 1 `read_end_of_day && !day1_shutdown`, Day 2 `accusation_made`) sets `returned_home_day1`, completes `head_home`, runs the evening deliveries, and tears down the work machines (tabs closed + `removeComputer` for nexacorp/devcontainer/chipinfra/erik-pc). |
| `lsblk`, `mount`, `umount` | `accepted_usb_drive` | accepting the anonymous USB tip Piper DM (`dm_anon`) on Day 2 morning |

`shutdown` is ungated and available on **every** computer (it's in `HOME_COMMANDS` and `DEVCONTAINER_COMMANDS`). Only two shutdowns are questline beats, both at home: the scripted end-of-Day-1 shutdown (handler branch requires `returned_home_day1 && !day1_shutdown`; advances to Day 2 via `runShutdownTransition`, which also closes all other tabs and `removeComputer`s any lingering work machines — no session survives the night) and the endgame credits shutdown (`read_board_debrief_day2`). Every other shutdown is **consequence-free**: at home it's a cosmetic reboot (`gameAction: { type: "reboot" }` → `runRebootTransition` in `useComputerTransitions.ts` — power-off lines, black screen, boot sequence, fresh prompt at `~`; closes other tabs but keeps `computerState`, and changes no flags/chapter/deliveries, so the in-game datetime is unchanged); on remote boxes it drops the SSH session back to the source machine (`getRemoteShutdownIncrementalLines` + `transitionTo`: nexacorp→home, devcontainer/chipinfra→nexacorp, erik-pc→chipinfra), reusing the same transition paths as `exit` — reconnecting finds the box back up, unchanged. Unlike `exit`, remote shutdown also sets `closeTabsForComputer`, which `useTerminal.ts` expands via `getConnectionClosure()` (`story/player.ts`) so sibling tabs on the rebooting box AND tabs chained through it drop too (a powered-off machine kills every SSH session to or through it: nexacorp down closes devcontainer/chipinfra/erik-pc tabs, chipinfra down closes erik-pc tabs). On nexacorp post-`accusation_made`, shutdown also fires the `exit_day2_logoff` event so powering off wraps Day 2 exactly like `exit`. The two home `command_executed: shutdown` flag triggers (`day1_shutdown`, `anon_tip_quest_started` in `storyFlags.ts`) carry `requiredFlags: ["returned_home_day1"]` so an early cosmetic reboot can never advance the day or surface the USB quest.
...
`coder_workspace_stopped` is a workspace-state flag rather than an unlock: set `true` by `coder stop`, `false` by `coder start`, absent = running. `coder ssh` is blocked when `true`; `coder stop` closes the workspace's tabs via `closeTabsForComputer` (expanded to the connection closure, so stopping chipinfra also drops erik-pc tabs).
...
`computeEffects()` auto-generates `file_read` events for commands registered with `readsFiles: true` (currently `cat`, `head`, `tail`, `grep`, `diff`, `wc`, `sort`, `uniq`, `file`, `less`, `source`, `pdftotext` — see the commands skill). Each file argument produces a `{ type: "file_read", detail: absolutePath }` event.
...
**Command availability.** `isCommandAvailable` (`src/engine/commands/availability.ts`) gives erik-pc the home PC's command set but **without the tutorial flag gates**: every command in `HOME_COMMANDS` or `HOME_GATED` is available unconditionally (Erik's work laptop is fully set up — a player who skipped Olive's optional `basic_tools_unlocked` challenge must not see "command not found" for `echo`/`whoami` there). Nexacorp-unlocked tools still carry over via `NEXACORP_GATED` flags; `DEVCONTAINER_ONLY` commands (`git`, `snow`, `dbt`) stay blocked. `exit` is always allowed on erik-pc so the player can return to chipinfra.
...
| home | nexacorp | — | `runExitToHome` (delegates to `runExitToParent("home")` soft disconnect unless `isEndOfDayExit()`; end-of-day runs the full teardown) |

`runExitToParent` is a **soft disconnect**: it repurposes only the active tab and never closes sibling tabs or removes `computerState`, so other sessions on the box being left keep working and reconnecting reattaches to the state as left. `runSshTransition("nexacorp")` mirrors this with a reattach branch: when `computerState.nexacorp` still exists (mid-shift soft disconnect), it skips the rebuild/boot/logo and all first-arrival side effects (chapter bump, Day 2 Snowflake rebuild, `seedImmediatePiper`, `ssh_day2` cascade) and drops straight to the shell. `ssh` never closes home tabs.
...
| t (ms) | Action |
|---|---|
| 0 | `setGamePhase("transitioning")`; set `terminated_for_misconduct` + `termination_reason` flags; close sibling nexacorp/devcontainer/chipinfra/erik-pc tabs so the player can't `<prefix>,N` away and keep working (prefix key is configurable via `~/.tmux.conf`, default Ctrl+Space) |
...
| 7900 | Reentry: rebuild home FS, `removeComputer` for nexacorp/devcontainer/chipinfra/erik-pc, repurpose active tab, deliver termination email, restore cursor, `writePrompt` |
