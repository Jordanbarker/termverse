...
> **Shared engine:** the pure pane model + helpers live in `@tt/core/terminal/paneTypes` and `PaneDividers` in `@tt/core/components/PaneDividers` (see the Architecture map below for the full core-vs-app split). These are reused by the second app `apps/term-crunch`, which ports the window/pane actions into its own lean store (`state/gameStore.ts`) and a trimmed renderer (`components/TabManager.tsx`) + a thin status-line wrapper (`components/TabBar.tsx`). Both games render the **same** status line and rename prompt from core: `@tt/core/components/TmuxStatusBar` (PREFIX indicator, `idx:label (paneCount)` tabs, modal takeover, app-injected `trailing` control), `@tt/core/terminal/windowLabel` (tab labels), and `@tt/core/terminal/useRenameWindowPrompt` (the `(rename-window)` keystroke handling). term-crunch mirrors the live game's multi-window UX: chords `<prefix> c/n/p/1-9/r` (new/cycle/jump/rename; `.`/`,` alias next/prev) alongside the pane chords `| - o x` + arrow focus. It also parses a **player-editable `~/.tmux.conf`** (Settings modal, default `DEFAULT_TMUX_CONF`) via the same `tmuxConfig.ts` parsers for prefix/theme/focus+resize binds (incl. `-r` repeat) — there's no home PC, the conf is stored in the game store and seeded into each challenge's fs; the built-in chords stay as fallbacks for keys the conf doesn't bind. Prefix/keep-alive logic mirrors `TabManager.tsx` (cross-window `liveIds`, `display:none` for non-active windows so buffers persist). Keep `paneTypes` helpers pure and store-agnostic so both apps can share them.
...
```
(The second app, `apps/term-crunch`, ports the same `@tt/core` model into its own `state/gameStore.ts` + `components/TabManager.tsx`/`TabBar.tsx`, and parses a Settings-editable `~/.zshrc`/`~/.tmux.conf` via the shared `tmuxConfig.ts`/`envParse.ts` parsers.)
