...
- **`apps/term-crunch` (`@tt/term-crunch`,** basePath `/terminal-turmoil/term-crunch`**)** — a second game built **entirely on `@tt/core`** to prove the engine-reuse pattern. It does **not** import terminal-turmoil's story code. **For its store/renderer/challenge framework, see `apps/term-crunch/CLAUDE.md`** (and the `apps/term-crunch:challenges` skill).
...
`.github/workflows/deploy.yml` builds **both** apps and assembles one GitHub Pages artifact (`_site/`): terminal-turmoil at `/terminal-turmoil/`, term-crunch nested at `/terminal-turmoil/term-crunch/`. basePath is the repo-name Pages path, so it is independent of the source directory.
