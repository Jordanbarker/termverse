...
An npm-workspace monorepo (`workspaces: ["packages/*", "apps/*"]`) holding a reusable terminal engine and two games built on it. The repo (and its GitHub Pages basePath) is **termverse**; the narrative game is **termoil**. Don't conflate the two: termverse is the platform, termoil is one game on it.
...
- **`apps/termoil` (`@tt/termoil`,** basePath `/termverse/termoil`**)** — the narrative game (a workplace-mystery Linux/terminal teacher). App code in `apps/termoil/src/`; play-testing harness in `apps/termoil/scripts/`. **For its architecture, story/engine split, computers, panes, and command gating, see `apps/termoil/CLAUDE.md`** (and the `apps/termoil:*` directory-scoped skills).
- **`apps/term-crunch` (`@tt/term-crunch`,** basePath `/termverse/term-crunch`**)** — a second game built **entirely on `@tt/core`** to prove the engine-reuse pattern. It does **not** import termoil's story code. **For its store/renderer/challenge framework, see `apps/term-crunch/CLAUDE.md`** (and the `apps/term-crunch:challenges` skill).
